...
public class ChessController implements Controller {
	...
	public boolean checkMate(boolean turn){
		...
		for(int i = xmin; i <= xMAX; i++)
			for(int j = ymin; j <= yMAX; j++){
				if(Table[i][j].getColor().equals(Teams.EMPTY)){
					if(chessScan(i, j, color, invor)){
						c = true;
						result++;
					}
				}else if(Table[i][j].getColor().equals(invor)){
					result++;
				}
			}
	...
	private boolean chessScan (int xp, int yp, Teams color, Teams invor){
		model.getController().setVariable(0,true);
		for(int i=0; i<8; i++)
			for(int j=0; j<8; j++)
				if(Table[i][j].getColor().equals(color))
					if(Table[i][j].mkMove(i, j, xp, yp, model)){
						model.getController().setVariable(0,false);
						if((!jumpEnemy(i,j,invor)) && (!fightEnemy(i,j,invor)))
							return true;
					}
		model.getController().setVariable(0,false);
		return false;
	...
	private boolean jumpEnemy (int x, int y, Teams color){
		...
		for(int i=0; i<8; i++)
			for(int j=0; j<8; j++)
				if(Table[i][j].getColor().equals(color))
					if(Table[i][j].getName().equals(Piece.PAWN)){
						int e = (color).equals(Teams.WHITE) ? -1 : 1;
						if((y == j+1 || y == j-1) && x == i+e){
							model.getController().setVariable(0,false);
							Table[x][y] = temp;
							return true;	
						}
					}else if(Table[i][j].mkMove(i, j, x, y, model)){
	...
	private boolean fightEnemy (int x, int y, Teams color){
		...
		for(int i=x+mx,j=y+my; i!=xk && j!=yk; i+=mx, j+=my)
				//System.out.println(i+" "+j);
				for(int a=0; a<8; a++)
					for(int b=0; b<8; b++)
						if(Table[a][b].getColor().equals(color) && (!(Table[a][b].getName().equals(Piece.KING)))){
