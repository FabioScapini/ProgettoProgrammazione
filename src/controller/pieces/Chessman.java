...
public abstract class Chessman {
	...
	public void jump(int x, int y, int x1, int y1, ChessModel model){
		...
		if(model.getController().getVariable(2) && Table[x][y].getName().equals(Piece.KING)){
	...
	public void movement(int ax, int ay, int bx, int by, ChessModel model){
		...
		if(Table[bx][by].getName().equals(Piece.KING))
			model.getController().setKing(Table[bx][by].getColor(), bx, by);
		else if(Table[bx][by].getName().equals(Piece.PAWN)){
			((Pawn)Table[bx][by]).notFirst();
		}
	...
	public boolean is_stalemate(ChessModel model){
		...
		for(int i=0; i<8; i++)
			for(int j=0; j<8; j++){
				if(model.getTable()[i][j].getName().equals(Piece.EMPTYMAN))
					++empty;
				else if (model.getTable()[i][j].getName().equals(Piece.BISHOP))
					++bishop;
				else if (model.getTable()[i][j].getName().equals(Piece.KNIGHT))
					++knight;
			}
	...
	public boolean equals (Object other){
		...
		return (other instanceof Chessman)
			&& ((Chessman)other).getName().equals(name)
			&& ((Chessman)other).getColor().equals(color);		
	}
