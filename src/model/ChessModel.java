...
public class ChessModel implements Model {
	...
	private void antiCheck(int x, int y, int x1, int y1){
		...
		if(OriginalTable[xk][yk].getName().equals(Piece.KING))
	...
	public boolean isWrongTurn(int x, int y){
		if((is_white_turn && OriginalTable[x][y].getColor().equals(Teams.BLACK))||
				(!is_white_turn && OriginalTable[x][y].getColor().equals(Teams.WHITE))){
			new WrongRoundPopUp();
			return true;
		}
		return false;
	...
	private int teamsToInt(Teams color){
		return color.equals(Teams.WHITE) ? 0 : 1;
	}
	...
	public void checkAdvancement(Teams color){
		...
		int i = color.equals(Teams.WHITE) ? 0 : 7;
		int x = 0;
		int y = 0;
		boolean check = false;
		
		for(int j=0; j<8; j++){
			if((OriginalTable[i][j]).getName().equals(Piece.PAWN)){
				check = true;
				x = i;
				y = j;
				break;
	...
	public void setController(ChessController other){
		controller = other;
	}
