...
public class TileListener implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {				
		...
		if(!model.gameIsStopped()){
			...
			if(model.getTable()[x][y].getName().equals(Piece.EMPTYMAN)){
				clickCount = 0;
			}else if(model.isWrongTurn(x,y))
				clickCount = 0;
			else{
				button.setBackground(model.getTable()[x][y].getColor().equals(Teams.WHITE) ?
	...
	private void possible_moves(){
		...
		for(int i=0; i<8; i++)
			for(int j=0; j<8; j++){
				if((model.getTable()[x][y].mkMove(x, y, i, j, model)
						||model.getController().getVariable(1))
						&&(!model.makeACheck()))
					if((i+j) % 2 != 0)
